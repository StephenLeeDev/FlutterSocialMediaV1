...
class _CreatePostScreenState extends State<CreatePostScreen> {
  ...
  Future<List<XFile>> pickImageFromCamera() async {
    final XFile? image = await imagePicker.pickImage(source: ImageSource.camera, maxHeight: maxImageLength, maxWidth: maxImageLength);
  ...
  Future<List<XFile>> pickImagesFromGallery() async {
    final List<XFile> images = await imagePicker.pickMultiImage(maxHeight: maxImageLength, maxWidth: maxImageLength);

    /// It prints image file's information
    if (kDebugMode) {
      for (int i = 0; i < images.length; i++) {
        await printImageInfo(fileName: images[i].name, file: File(images[i].path));
      }
    }
